...
export class Ludo {
    ...
    loadGameState(token) {
        fetch(`http://localhost:3000/load/${token}`)
            .then(response => {
                if (response.ok) {
            ...
            .then(gameData => {
                ...
                this.currentPositions = gameData.currentPositions;
                this.turn = gameData.turn;
                this.state = gameData.state;
                this.diceValue = gameData.diceValue;
    ...
    initializeBoard() {
        Object.keys(this.currentPositions).forEach(player => {
            this.currentPositions[player].forEach((position, index) => {
                UI.setPiecePosition(player, index, position);
            });
        });
    }
